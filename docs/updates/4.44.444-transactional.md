...
In Craft 3, an author could add any From Name and From Email address when creating an email.

In Craft 4, From Name and Email settings are created ahead of time in the Mailer Settings and assigned to an Email Type. While creating an email, an author must now select the Sender info they wish to use from the pre-defined Mailer Settings.

In the migration we try to migrate all the settings to a 'Custom Mailer', however, Mailer Settings only allow a given email to be used once, so the migration will only migrate a single Name/Email pair to the new Custom Mailer. In the scenario you have configured multiple emails to use the same From Email but a different From Name, you may want to update your Mailer or selected Mailer Settings in a given Email after the migration.
